...
class FunctionCallSpec extends FlatSpec {
  ...
  "Functions" should "be considered values" in {
    val f = "f"
    val x = "x"
    val e1 = Function(None, List((x, (PConst, TNumber))), Some(TNumber), Var(x))
    val e2 = Function(Some(f), List((x, (PConst, TNumber))), Some(TNumber), Var(x))
    assert(isValue(e1))
    assert(isValue(e2))
  } 
  ...
  "Call" should "evaluate a function using small-step semantics" in {
    val f = "f"
    val x = "x"
    val e1 = Function(None, List((x, (PConst, TNumber))), Some(TNumber), Binary(Plus, Var(x), N(1)))
    val e2 = N(2)
    val e3 = iterateStep(Map.empty, Call(e1, List(e2)))
